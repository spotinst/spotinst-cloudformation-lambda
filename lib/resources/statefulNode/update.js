        ...
        request(updateStatefulNodeOptions, function (err, res, body) {
            spotUtil.validateResponse({
                ...
                successCb: function () {
                    ...
                    let shouldPause = spotUtil.parseBoolean(updatePolicy.shouldPause);
                    let shouldResume = spotUtil.parseBoolean(updatePolicy.shouldResume);
                    let shouldRecycle = spotUtil.parseBoolean(updatePolicy.shouldRecycle);

                    if (shouldPause) {
                        spotUtil.pauseStatefulNode({
                            event: event,
                            context: context,
                            refId: refId,
                            token: tc.token
                ...
                failureCb: function() {
                    ...
                    console.log("Can't update Stateful Node, error: " + errMsg);
                }
            })
        })
    })
