...
let destroy = async(function(err, event, context) {
  spotUtil.getToken(event, function(err, token) {
    ...
    if(spotUtil.checkAutoTag(event)){
      refId = await((cb, res)=>{
        asyncLib.retry({times: 10, interval: 1000}, getElastigroupFromTags.bind({event:event, token:token}), (err, res)=>{
          if(err){
    ...
    request(deleteOptions, function(err, res, body) {
      spotUtil.validateResponse({
        ...
        successCb: function(spotResponse) {
          util.done(err, event, context, spotResponse.body);
        },
 ...
 */
function getElastigroupFromTags(cb){
  let autoTags = spotUtil.generateTags(this.event)

  var getAllGroupsOptions = {
    url:"https://api.spotinst.io/aws/ec2/group",
    method:"GET",
    qs:{
      accountId: spotUtil.getSpotinstAccountId(this.event)
    },
    headers:{
        'Content-Type' : 'application/json',
