...
let destroy = async function(err, event, context) {
  ...
  let shouldDeleteAmiImages         = spotUtil.parseBoolean(deletePolicy.shouldDeleteAmiImages);
  ...
  var deleteOptions = {
    ...
    json: {
      ...
      amiBackup: {
        shouldDeleteAmiImages:         shouldDeleteAmiImages
      }
    },
    headers: {
...
function getElastigroupFromTags(event, token){
  return new Promise((resolve, reject)=>{
    ...
    async.retry({times: 7, interval: 1000}, getAllGroups, (err, res)=>{
      if(err){
        return reject(err)
      }else{
        return resolve(res)
