...
function create(err, event, context) {
 	spotUtil.getTokenAndConfig(event, function(err, tc) {
		...
		let createOceanLaunchSpecOptions = {
			uri:     "https://api.spotinst.io/ocean/aws/k8s/launchSpec",
			method:  "POST",
			qs:      {
		...
		console.log('Creating ocean launch spec: ' + JSON.stringify(tc.config, null, 2))
		...
		request(createOceanLaunchSpecOptions, function(err, res, body) {
			spotUtil.validateResponse({
				...
				successCb: function(spotResponse) {
					console.log('Ocean Launch Spec Creation Success: ' + JSON.stringify(body, null, 2));
