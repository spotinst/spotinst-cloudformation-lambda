...
module.exports.getTokenAsync = function(obj){
  return new Promise((resolve, reject)=>{
    ...
    var config = obj.event.ResourceProperties || obj.event;
...
module.exports.validateOceanCluster = function(clusterId, token, event, context) {
  ...
  request(getGroupOptions, function(err, res, body) {
    module.exports.validateResponse({
      ...
      context:   context,
      successCb: function(spotResponse) {
        console.log("The ocean cluster does exist, fail the delete.");
        // Return failed to delete (cause the group does exist)
        util.done("Validate Failed", event, context);
      },
      failureCb: function(spotResponse) {
        ...
        if(spotResponse.res.statusCode === 404 || spotResponse.res.statusCode === 400) {
          ...
          console.log("The ocean cluster doesn't exist, set the delete as success.")
          try {
            body = JSON.parse(body);
          }
          catch(err) {
...
/**
 * This function rolls the Cluster. Used in update. currently for ECS Clusters only.
 *
 * @function
 * @name generateTags
 *
 * @param {Object} obj - Contains {event: event, context: context, updatePolicy: updatePolicy, refId: refId, token: tc.token, body: updatePolicy.rollConfig}
 */
module.exports.rollCluster = function(obj){
  ...
  if (resourceTypeCustom === "Custom::oceanEcs"){
    ...
    console.log("OceanEcs integration, preform cluster roll");
  }
  
  else if (resourceTypeCustom === "Custom::ocean"){
    url = "https://api.spotinst.io/ocean/aws/k8s/cluster/" + obj.refId + "/roll";
    console.log("OceanK8S integration, Preform cluster roll")
  }
